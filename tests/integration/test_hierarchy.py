...
class TestHierarchyCount:
    ...
    def n_tiers(self, request):
        return request.param

    def test_single_drude_mode(self, n_tiers):
        # K=1: single Drude pole, type_ltc='none'
        corr = noise_decomposition(Drude(eta=1.0, gamma_c=1.0), T=1.0, type_ltc='none')
        K = len(corr.phi_0)
        assert K == 1
        qme = _make_solver(corr, n_tiers)
        assert qme.qme_impl.get_n_hrchy() == comb(K + n_tiers, K)

    def test_two_mode_overdamped_brown(self, n_tiers):
    ...
    def test_three_mode_underdamped_brown_psd(self, n_tiers):
...
class TestStorageSize:
    ...
    @pytest.mark.parametrize("n_tiers", [1, 2, 3])
    def test_storage_size_is_n_hrchy_plus_one(self, n_tiers):
        corr = noise_decomposition(Drude(eta=1.0, gamma_c=1.0), T=1.0, type_ltc='none')
        qme = _make_solver(corr, n_tiers)
        n_hrchy = qme.qme_impl.get_n_hrchy()
        assert qme.storage_size() == n_hrchy + 1

    @pytest.mark.parametrize("n_tiers", [1, 2, 3])
    def test_rho_hierarchy_shape(self, n_tiers):
        corr = noise_decomposition(Drude(eta=1.0, gamma_c=1.0), T=1.0, type_ltc='none')
        qme = _make_solver(corr, n_tiers)
    ...
    @pytest.mark.parametrize("n_tiers", [1, 2, 3])
    def test_rho_is_view_of_hierarchy(self, n_tiers):
        corr = noise_decomposition(Drude(eta=1.0, gamma_c=1.0), T=1.0, type_ltc='none')
        qme = _make_solver(corr, n_tiers)
