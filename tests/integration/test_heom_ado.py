...
Reference value: space='liouville', h_order='C', liouville_order='C', truncation_depth=3,
...
def _run(space, h_order, liouville_order, truncation_depth=3, t_final=5.0, callback_dt=0.025):
...
# ---------------------------------------------------------------------------
# Consistency across truncation_depth for ado vs liouville
# ---------------------------------------------------------------------------

@pytest.mark.parametrize("truncation_depth", [1, 2, 3, 4])
@pytest.mark.parametrize("h_order,liouville_order", [
    pytest.param('C', 'C', id='CC'),
    pytest.param('F', 'F', id='FF'),
])
def test_ado_liouville_agreement_tiers(truncation_depth, h_order, liouville_order):
    rho_liou = _run('liouville', h_order, liouville_order, truncation_depth=truncation_depth)
    rho_ado  = _run('ado',       h_order, liouville_order, truncation_depth=truncation_depth)
    assert rho_ado == pytest.approx(rho_liou, abs=TOL), \
        f"truncation_depth={truncation_depth} h={h_order} l={liouville_order}: ado={rho_ado} liou={rho_liou}"
